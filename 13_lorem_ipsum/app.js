...
form.addEventListener('submit', (e)=>{
           e.preventDefault();                     // default behaviour ( A click on a form submit button – initiates its submission to the server.)
           const value = parseInt(amount.value);     // i/p value is in string 
           
           const random = Math.floor(Math.random() * text.length);
           if(isNaN(value) || value<=0 || value>9 ){                     // no value entered just submitt button was clicked.
              //  result.textContent = text[random];       // will work as we'll have only 1 text[] to display.
              result.innerHTML = `<p class="result"> ${text[random]} </p>`;
           }
           else{
              let tempText = text.slice(0,value);         // tempText = text[0 to value-1]
              tempText = tempText.map((item)=>{
                  return `<p class="result"> ${item} </p>`;
              });
              tempText = tempText.join("");
              /* let tempText = text.slice(0,value).map((item)=>{
                                                        return `<p class="result"> ${item} </p>`;
                                                    }).join("");
              */
...
;})
/* - working with forms 
     in console : strings -> black
                  number  -> blue

    - by default any value we i/p in i/p box is string 
    - validating inputs 
*/